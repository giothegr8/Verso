...
import { TERMS_OF_SERVICE_MD } from "../constants/termsOfService";
...
export default function TermsOfServiceModal({ isOpen, onClose, primaryLanguage }: TermsOfServiceModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div 
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-serif font-black text-earth dark:text-ivory tracking-tight">
                    {primaryLanguage === 'es' ? 'Términos de Servicio' : 'Terms of Service'}
                  </h2>
                  <p className="text-[10px] font-black text-earth-light/60 dark:text-lavender-muted uppercase tracking-widest">
                    {primaryLanguage === 'es' ? 'Reglas de uso de la app' : 'Rules for using the app'}
                  </p>
                </div>
              </div>
              <button 
